...
def _validate_yang_model(filename: Path, options: List[str]):
    ...
    # I think that in libyang, there's no notion of "canonical order" and what not, so we might need pyyang here
    # (but just for the test suite).
    ...
    proc = subprocess.run(
        ('pyang', '-p', os.pathsep.join((str(external_path()), str(model_path()))), *options, filename),
