...
def list_element_attr(element):
    ...
    Many parameters (location ones) look redondant -> this will be corrected with elements refactor
    from Andrea
    """
...
def power_difference_first_amp(base_amps, element_after_propagation, power_dbm, req_power, dp, pch_max):
...
def power_difference_subsequent_amps(base_amps, element_after_propagation, previous_amp_was_saturated,
                                     power_dbm, req_power, previous_deltap, pch_max):
    """ returns power difference after-before propagation according to the different cases:

    This depends on whether the previous amp was saturated, and whether this one is saturated.
    ...
    if hasattr(base_amps[element_after_propagation.uid], 'type_variety'):
        ...
        # this amp is possibly saturated
        if previous_amp_was_saturated:
            # this amp might have been saturated, and so has the previous one
            return \
                min(pch_max, req_power + element_after_propagation.delta_p) -\
                min(pch_max, req_power + previous_deltap) -\
                min(pch_max, power_dbm + element_after_propagation.delta_p) +\
                min(pch_max, power_dbm + previous_deltap)
        # this one might be saturated, but not the previous one
        return \
            min(pch_max, req_power + element_after_propagation.delta_p) -\
            min(pch_max, req_power + previous_deltap) -\
            min(pch_max, power_dbm + element_after_propagation.delta_p) +\
...
def test_booster_target_power_and_gain(power_dbm, req_power, amps_with_adjusted_delta_p):
    ...
    - in power mode only:
        ...
        so gain of booster should be reduced/increased by the exact power difference
        the rest of the amplifier have unchanged gain
        except if the increase leads to total_power above amplifier max power
    ...
    for pth in paths:
        ...
        for i, element in enumerate(pth[1:-1]):
            ...
            if element_is_first_amp and hasattr(base_amps[element.uid], 'type_variety'):
                # previous_amp_was_saturated state is recorded now to be used for analysis of the next amp in the path
            ...
            for key in list_element_attr(element):
                ...
                if not isinstance(parameters, (EdfaOperational, EdfaParams, FiberParams)):
                    if not key == 'effective_gain':
                        ...
                        dp = element.out_voa if element.uid not in amps_with_adjusted_delta_p else element.out_voa + 1
                        if element_is_first_amp:
                            assert element.effective_gain - element_copy.effective_gain ==\
                                pytest.approx(power_difference_first_amp(base_amps, element, power_dbm,
                                                                         req_power, dp, pch_max),
                                              abs=1e-2)
                        else:
                            ...
                            assert element.effective_gain - element_copy.effective_gain == \
                                pytest.approx(power_difference_subsequent_amps(
                                    base_amps, element, previous_amp_was_saturated, power_dbm, req_power,
                        ...
                        if hasattr(base_amps[element.uid], 'type_variety'):
                            # previous_amp_was_saturated state is recorded now to be used for analysis of the next amp
                            # in the path
                            previous_amp_was_saturated = False
