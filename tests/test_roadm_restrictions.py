...
from gnpy.core.utils import db2lin, dbm2watt, watt2dbm
...
def create_per_oms_request(network, eqpt, req_power):
    ...
    for trx in trxs:
        ...
        for degree in roadm.per_degree_pch_out_dbm.keys():
            ...
            carrier = {key: getattr(req, key) for key in ['power', 'baud_rate', 'roll_off', 'spacing', 'tx_osnr']}
            carrier['label'] = ""
            ...
            for key in list_element_attr(element):
                if not isinstance(getattr(element, key),
                    ...
                    if not key == 'effective_gain':
                        ...
                        # gain is slightly less than the one computed during design for the noiseless reference, because
                        # during propagation, noise has accumulated, additing to signal.
                        ...
                        if element_is_first_amp:
                            # if element is first amp on path, then it is the one that will saturate if req_power is too high
