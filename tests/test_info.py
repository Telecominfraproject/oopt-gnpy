...
def test_dim_reshape():
    ...
    with pytest.raises(SpectrumError, match='Dimension mismatch field: field_name.'):
        dimension_reshape(value=[1, 2, 3], dimension=1, name='field_name')
    with pytest.raises(SpectrumError, match='Missing mandatory field: field_name.'):
...
def test_create_arbitrary_spectral_information():
    ...
    with pytest.raises(SpectrumError,
        ...
        create_arbitrary_spectral_information(frequency=frequency, baud_rate=baud_rate)
