...
from gnpy.tools.convert import convert_file, corresp_names
...
def test_target_pch_out_db_global(case):
    ...
    for elem in json_network['elements']:
        if 'type' in elem.keys() and elem['type'] == 'Roadm' and 'params' in elem.keys() and 'per_degree_pch_out_db' in elem['params']:
