...
from gnpy.core.utils import lin2db, automatic_nch, db2lin, dbm2watt, powerdbm2psdmwperghz, watt2dbm
...
def create_rq(equipment, srce, dest, bdir, nd_list, ls_list, mode, spacing, power_dbm):
...
def test_initial_spectrum(mode, spacing, power_dbm):
    ...
    req = create_rq(equipment, 'trx Brest_KLA', 'trx Vannes_KBE', False, ['trx Vannes_KBE'], ['STRICT'], mode, spacing, power_dbm)
...
def test_target_psd_out_mWperGHz(with_initial_spectrum):
    ...
    equipment = load_equipment(EQPT_FILENAME)
    network = net_setup(equipment)
    req = create_rq(equipment, 'trx Brest_KLA', 'trx Vannes_KBE', False, ['trx Vannes_KBE'], ['STRICT'], 'mode 1', 50e9, 0)
    ...
    setattr(equipment['Roadm']['default'], 'target_psd_out_mWperGHz', powerdbm2psdmwperghz(-20, 32e9, 0.15))
...
def ref_network():
    equipment = load_equipment(EQPT_FILENAME)
    network = net_setup(equipment)
    req0 = create_rq(equipment, 'trx Brest_KLA', 'trx Vannes_KBE', False, ['trx Vannes_KBE'], ['STRICT'], 'mode 1', 50e9, 0)
    path0 = compute_constrained_path(network, req0)
    infos_expected = propagate(path0, req0, equipment)
    return network
...
def test_target_psd_out_mWperGHz_deltap(deltap):
    ...
    equipment = load_equipment(EQPT_FILENAME)
    network = net_setup(equipment)
    req = create_rq(equipment, 'trx Brest_KLA', 'trx Vannes_KBE', False, ['trx Vannes_KBE'], ['STRICT'], 'mode 1', 50e9, deltap)
    temp = [
            {
            "f_min": 191.35e12,     # align f_min , f_max on Voyager f_min, f_mix and not SI !
            "f_max":196.1e12,
...
def test_equalization(case, deltap, target, mode, spacing):
    """check that power target on roadm is correct for this cases check on booster
    ...
    if case == 'SI':
        ...
        psd_roadms = [r for r in network.nodes() if r.uid in roadms]
    ...
    for i, el in enumerate(path):
        if isinstance(el, Roadm):
            ...
            if case in ['SI', 'nodes', 'degrees']:
                assert_allclose(powerdbm2psdmwperghz(watt2dbm(si.signal + si.ase + si.nli), si.baud_rate, si.roll_off),
...
def test_power_option(req_power):
    ...
    setattr(equipment['Roadm']['default'], 'target_psd_out_mWperGHz', powerdbm2psdmwperghz(-20, 32e9, 0.15))
    network = net_setup(equipment)
    req = create_rq(equipment, 'trx Brest_KLA', 'trx Vannes_KBE', False, ['trx Vannes_KBE'], ['STRICT'],
