...
def test_wrong_amp(caplog, error, json_data, expected_msg):
    """
    """
    with pytest.raises(EquipmentConfigError):
...
def test_json_request(caplog, error, json_data, expected_msg):
    """
...
def test_json_network(caplog, error, json_data, expected_msg):
    """
    Check that a missing key is correctly raisong the logger
    """
    equipment = load_equipment(EQPT_FILENAME)
