...
# Get the path of the default equipment file and load it at startup:
eqpt_config_path = Path(__file__).resolve().parents[2] \
                    .joinpath('examples', 'eqpt_config.json')
equipment_global = load_equipment(eqpt_config_path)
...
def load_equipment_file(content, filename):
    ...
    try:
        equipment = equipment_from_json(equipment_json, eqpt_config_path)
...
def load_network_file(content, auto_design, equipment, filename):
    ...
    try:
        if equipment:
            equipment = equipment_from_json(equipment, eqpt_config_path)
        else:
            # if no equipment file has been uploaded, use the default config:
            equipment = deepcopy(equipment_global)
