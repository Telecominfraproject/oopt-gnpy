...
def _create_context() -> ly.Context:
    ...
    ctx = ly.Context(str(model_path()) + os.pathsep + str(external_path()),
                     ...
                     | ly.ContextOptions.NoYangLibrary)
...
class ErrorMessage(NamedTuple):
    what: str
    where: str
    # FIXME: separate data path, schema path and line number when upstream supports that
    # (which is not gonna happen too soon because it requires another ABI change)
