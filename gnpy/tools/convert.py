...
def sanity_check(nodes: List[Node], links: List[Link], roadms: List[Roadm],
    ...
    if bad_roadm:
        msg = 'XLS error: ' \
            ...
            + _format_items(f'{item[0]} -> {item[1]}' for item in bad_roadm)
        raise NetworkTopologyError(msg)
    ...
    if bad_roadm_degrees:
        msg = 'XLS error: ' \
            ...
            + _format_items(f'{item[0]} -> {item[1]}' for item in bad_roadm_degrees)
        raise NetworkTopologyError(msg)
    return nodes, links
...
def create_nodes_uid(links: List[Link], eqpts: List[Eqpt], nodes: List[Node],
    ...
    for fiber in links:
        ...
        pair_id = f'-pair-{fiber.pair_id}' if fiber.pair_id not in ['0', None] else ''
        fiber.east_uid = f'fiber ({fiber.from_city} -> {fiber.to_city}){cable_id}{pair_id}'
        cable_id = f'-{fiber.west_cable}' if fiber.west_cable else '-'
        pair_id = f'-pair-{fiber.pair_id}' if fiber.pair_id not in ['0', None] else ''
        fiber.west_uid = f'fiber ({fiber.to_city} -> {fiber.from_city}){cable_id}{pair_id}'
    for eqpt in eqpts:
        pair_id = f'-pair-{eqpt.pair_id}' if eqpt.pair_id not in ['0', None] else ''
        eqpt.east_uid = f'east edfa in {eqpt.from_city} to {eqpt.to_city}{pair_id}'
        pair_id = f'-pair-{eqpt.pair_id}' if eqpt.pair_id not in ['0', None] else ''
        eqpt.west_uid = f'west edfa in {eqpt.from_city} to {eqpt.to_city}{pair_id}'
    for node in nodes:
        pair_id = f'-pair-{node.pair_id}' if node.pair_id not in ['0', None] else ''
        if node.node_type.lower() == 'ila' and node.city not in eqpts_by_city:
            node.west_uid = f'west edfa in {node.city}{pair_id}'
            node.east_uid = f'east edfa in {node.city}{pair_id}'
        if node.node_type.lower() == 'fused':
            node.west_uid = f'west fused spans in {node.city}{pair_id}'
            node.east_uid = f'east fused spans in {node.city}{pair_id}'
...
def fiber_link(from_city: str, to_city: str, links_by_city: DefaultDict[str, List[Link]], pair_id) -> str:
    ...
    pair_id = f'-pair-{link.pair_id}' if link.pair_id not in ['0', None] else ''
    if link.from_city == from_city:
        fiber = f'fiber ({link.from_city} -> {link.to_city})-{link.east_cable}{pair_id}'
    else:
        fiber = f'fiber ({link.to_city} -> {link.from_city})-{link.west_cable}{pair_id}'
    return fiber
