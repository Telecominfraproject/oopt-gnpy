...
def sanity_check(nodes, links, nodes_by_city, links_by_city, eqpts_by_city):
    ...
    if unreferenced_nodes:
        raise NetworkTopologyError(f'{ansi_escapes.red}XLS error:{ansi_escapes.reset} '
            f'The following nodes are not referenced from the {ansi_escapes.cyan}Links{ansi_escapes.reset} sheet. '
    ...
    wrong_eqpt_to = [n.to_city for destinations in eqpts_by_city.values() for n in destinations if n.to_city not in nodes_by_city]
    wrong_eqpt = wrong_eqpt_from + wrong_eqpt_to
    if wrong_eqpt:
        raise NetworkTopologyError(f'{ansi_escapes.red}XLS error:{ansi_escapes.reset} '
            f'The {ansi_escapes.cyan}Eqpt{ansi_escapes.reset} sheet refers to nodes that are not defined '
            f'in the {ansi_escapes.cyan}Nodes{ansi_escapes.reset} sheet:\n'
...
def parse_excel(input_filename):
    ...
    if bad_links:
        raise NetworkTopologyError(f'{ansi_escapes.red}XLS error:{ansi_escapes.reset} '
            f'The {ansi_escapes.cyan}Links{ansi_escapes.reset} sheet references nodes that are not defined '
            f'in the {ansi_escapes.cyan}Nodes{ansi_escapes.reset} sheet:\n'
