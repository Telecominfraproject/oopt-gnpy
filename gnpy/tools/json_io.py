...
from gnpy.core.utils import automatic_nch, automatic_fmax, merge_amplifier_restrictions, Carrier, merge_equalization
...
def _spectrum_from_json(json_data):
    ...
    >>> json_data = {'SI': \
        [{'f_min': 0, 'f_max': 2, 'slot_width': 1, 'baud_rate': 2.5e9, 'roll_off': 0.15, 'delta_pdb': 1, 'tx_osnr': 45},\
        {'f_min': 3, 'f_max': 4, 'slot_width': 1, 'baud_rate': 3e9, 'roll_off': 0.15},\
        {'f_min': 5.25, 'f_max': 9, 'slot_width': 1.5, 'baud_rate': 3e9, 'roll_off': 0.15},\
        {'f_min': 11, 'f_max': 20, 'slot_width': 2, 'baud_rate': 4e9, 'roll_off': 0.15}]}
    >>> spectrum = _spectrum_from_json(json_data['SI'])
    >>> for k, v in spectrum.items():
    ...     print(f'{k}: {v}')
    ... 
    0: Carrier(delta_pdb=1, baud_rate=2500000000.0, slot_width=1, roll_off=0.15, tx_osnr=45, label='0-2.5')
    1: Carrier(delta_pdb=1, baud_rate=2500000000.0, slot_width=1, roll_off=0.15, tx_osnr=45, label='0-2.5')
    2: Carrier(delta_pdb=1, baud_rate=2500000000.0, slot_width=1, roll_off=0.15, tx_osnr=45, label='0-2.5')
    3: Carrier(delta_pdb=0, baud_rate=3000000000.0, slot_width=1, roll_off=0.15, tx_osnr=40, label='1-3.0')
    4: Carrier(delta_pdb=0, baud_rate=3000000000.0, slot_width=1, roll_off=0.15, tx_osnr=40, label='1-3.0')
    5.25: Carrier(delta_pdb=0, baud_rate=3000000000.0, slot_width=1.5, roll_off=0.15, tx_osnr=40, label='2-3.0')
    6.75: Carrier(delta_pdb=0, baud_rate=3000000000.0, slot_width=1.5, roll_off=0.15, tx_osnr=40, label='2-3.0')
    8.25: Carrier(delta_pdb=0, baud_rate=3000000000.0, slot_width=1.5, roll_off=0.15, tx_osnr=40, label='2-3.0')
    11: Carrier(delta_pdb=0, baud_rate=4000000000.0, slot_width=2, roll_off=0.15, tx_osnr=40, label='3-4.0')
    13: Carrier(delta_pdb=0, baud_rate=4000000000.0, slot_width=2, roll_off=0.15, tx_osnr=40, label='3-4.0')
    15: Carrier(delta_pdb=0, baud_rate=4000000000.0, slot_width=2, roll_off=0.15, tx_osnr=40, label='3-4.0')
    17: Carrier(delta_pdb=0, baud_rate=4000000000.0, slot_width=2, roll_off=0.15, tx_osnr=40, label='3-4.0')
    19: Carrier(delta_pdb=0, baud_rate=4000000000.0, slot_width=2, roll_off=0.15, tx_osnr=40, label='3-4.0')
    """
    spectrum = {}
    ...
    for index, part in enumerate(json_data):
        ...
        # first partition min frequency is f_min - spacing / 2
        ...
        if previous_part_max_freq <= (part['f_min'] - part['slot_width'] / 2):
            ...
            for current_freq in arange(part['f_min'],
                                       ...
                                       part['slot_width']):
                spectrum[current_freq] = Carrier(**{k: v for k, v in part.items() if k not in ['f_min', 'f_max']})
            previous_part_max_freq = current_freq + part['slot_width'] / 2
        else:
            print(previous_part_max_freq, (part['f_min'] - part['slot_width'] / 2))
