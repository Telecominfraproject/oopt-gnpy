...
def requests_from_json(json_data, equipment):
    ...
    for req in json_data['path-request']:
        ...
        try:
            ...
            params['nb_channel'] = automatic_nch(f_min, f_max_from_si, params['spacing'])
        params['effective_freq_slot'] = req['path-constraints']['te-bandwidth'].get('effective-freq-slot', [{'N':None, 'M': None}])
        try:
...
def _check_one_request(params, f_max_from_si):
    ...
    if params['trx_mode'] is not None and params['effective_freq_slot'] is not None:
        ...
        for slot in slots:
            nb_of_channels = nb_of_channels + slot['M'] // per_channel_m if slot['M'] is not None and nb_of_channels is not None else None
