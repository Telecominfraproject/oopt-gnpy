...
class OMS:
    ...
    def update_spectrum(self, f_min, f_max, guardband=0.15e12, existing_spectrum=None, grid=0.00625e12):
        ...
        """
        self.spectrum_bitmap = Bitmap(f_min=f_min, f_max=f_max, grid=grid, guardband=guardband, bitmap=existing_spectrum)
...
def determine_slot_numbers(test_oms, requested_n, required_m, per_channel_m):
    ...
    i = per_channel_m
    while (freq_availability[center_i - i:center_i + i] == [1] * (2 * i) and
                freq_index[center_i - i] >= freq_index_min
...
def pth_assign_spectrum(pths, rqs, oms_list, rpths):
    ...
    for pth, rq, rpth in zip(pths, rqs, rpths):
        if hasattr(rq, 'blocking_reason'):
            ...
            selected_n, selected_m, remaining_slots_to_serve = compute_n_m(required_m, rq, path_oms, oms_list, per_channel_m)
