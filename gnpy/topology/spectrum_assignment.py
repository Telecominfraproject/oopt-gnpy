                ...
                if oms.el_list[0].degree_association:
                    ...
                    if reversed_oms_egress_uid:
                        try:
                            ...
                            raise NetworkTopologyError(f'Parralel OMSes: degrees association {reversed_oms_egress_uid} '
                                                       + f'not found in candidate OMSes {candidate_omses_names}')
                    else:
                        raise NetworkTopologyError(f'Parralel OMSes: degree {oms.el_id_list[1]} not found in roadm'
                                                   + f'{oms.el_list[0].uid} degrees association')
                else:
                    raise NetworkTopologyError(f'Parralel OMSes: OMS {oms.el_id_list[1]} has more than one candidate for'
                                               + ' reverse OMS. Correct reversed OMS can not be identified. Please add '
                                               + 'degree-association in the end points')
            if not has_reversed:
