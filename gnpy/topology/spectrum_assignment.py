                    ...
                    if reversed_oms_egress_uid:
                        try:
                            ...
                            raise NetworkTopologyError(
                                f'Parallel OMSes: degrees association {reversed_oms_egress_uid} not found '
                                + f'in candidate OMSes {candidate_omses_names}')
                    else:
                        raise NetworkTopologyError(
                            f'Parallel OMSes: degree {oms.el_id_list[1]} not found'
                            + f' in roadm {oms.el_list[0].uid} degrees association')
                else:
