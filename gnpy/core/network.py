        ...
        while not (isinstance(node, elements.Roadm) or isinstance(node, elements.Transceiver)):
            ...
            if isinstance(node, elements.Edfa):
                ...
                if node.params.type_variety == '':
                    ...
                    if power_mode:
                        power_reduction = min(0, equipment['Edfa'][node.params.type_variety].p_max - (pref_total_db + dp))
                    else:
                        pout = pref_total_db + prev_dp - node_loss - prev_voa + gain_target
                        power_reduction = min(0, equipment['Edfa'][node.params.type_variety].p_max - pout)
                    dp += power_reduction
                    gain_target += power_reduction
