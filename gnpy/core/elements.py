...
class Fiber(_Node):
    ...
    def loss_coef_func(self, frequency):
        ...
        or made frequency-dependent defined via a dictionary-model per instance
        in the topology-file, or via a list-model in the equipment-file.

        If a list-model is declared, then the legacy 'loss_coef' scalar is used to
        scale the provided list-model for the given 'loss_coef_offset_frequency' such
        ...
        if self.params.loss_coef.size > 1:
            ...
            # scale the the loss_coef list-model using the static loss coef for
            # estimating the offset of the list-model at the loss coef offset frequency
