...
class Roadm(_Node):
    ...
    def __str__(self):
        ...
        return '\n'.join([f'{type(self).__name__} {self.uid}',
                          ...
                          f'  total pch (dBm):  {pretty_summary_print(per_baudrate_summary(self.pch_out_dbm, self.baud_rate))}'])
    ...
    def propagate(self, spectral_info, degree):
        ...
        if self.target_pch_out_dbm:
            ...
            per_degree_pch = psd2powerdbm(self.per_degree_pch_psd[degree], spectral_info.baud_rate) \
                if degree in self.per_degree_pch_psd else psd2powerdbm(self.target_psd_out_mWperGHz, spectral_info.baud_rate)
            ref_per_degree_pch = psd2powerdbm(self.per_degree_pch_psd[degree], ref_baud_rate) \
                if degree in self.per_degree_pch_psd else psd2powerdbm(self.target_psd_out_mWperGHz, ref_baud_rate)
        ...
        if self.target_pch_out_dbm:
            ...
            # if all carriers are identical baud_rate, the target power is the min power
