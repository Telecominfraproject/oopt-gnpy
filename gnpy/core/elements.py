    ...
    def _calc_mode(self, spectral_info):
        ...
        for carrier in spectral_info.carriers:
            ...
            for mode in list_modes:
                ...
                cd_penalty = interp([mode['cd_penalty']['cd_threshold'], mode['cd_limit']],
                                    [0, mode['cd_penalty']['osnr_penalty']], carrier.chromatic_dispersion) \
                    if carrier.chromatic_dispersion > mode['cd_penalty']['cd_threshold'] else 0

                dgd_penalty = interp([mode['dgd_penalty']['dgd_threshold'], mode['dgd_limit']],
                                     [0, mode['dgd_penalty']['osnr_penalty']], carrier.pmd) \
                    if carrier.pmd > mode['pmd_penalty']['pmd_threshold'] else 0

                snr = 10*log10(carrier.power.signal / (carrier.power.ase + carrier.power.nli))
                if mode['OSNR'] < snr - cd_penalty - dgd_penalty:
