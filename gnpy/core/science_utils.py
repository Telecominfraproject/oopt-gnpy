...
def raised_cosine(frequency, channel_frequency, channel_baud_rate, channel_roll_off):
    ...
    raised_cosine_mask[cosine_condition] = 0.5 * (
                1 + cos(pi * ts / channel_roll_off * (abs(base_frequency[cosine_condition]) - pass_band)))
...
class NliSolver:
    ...
    @staticmethod
    def compute_nli(spectral_info: SpectralInformation, srs: StimulatedRamanScattering, fiber, spm_weight=SPM_WEIGHT,
        ...
        if 'gn_model_analytic' == sim_params.nli_params.method:
            eta = \
                NliSolver._gn_analytic(baud_rate, delta_frequency, alpha, beta2, gamma, length, spm_weight, xpm_weight)
            cut_power = outer(signal, ones(nch))
            pump_power = outer(ones(nch), signal)
            ...
            cut_power = outer(signal[cut_indices], ones(nch))
            pump_power = outer(ones(cut_indices.size), signal)
            cut_baud_rate = outer(spectral_info.baud_rate[cut_indices], ones(nch))

            eta = NliSolver._ggn_spectrally_separated(cut_indices, frequency, signal, baud_rate, roll_off, slot_width,
                                                      srs, alpha, beta2, beta3, f_ref_beta, gamma, spm_weight,
                                                      xpm_weight)
            g_nli = eta * cut_power * pump_power**2 / cut_baud_rate
            g_nli = sum(g_nli, 1)
            g_nli = interp(frequency, frequency[cut_indices], g_nli)
    ...
    @staticmethod
    def _gn_analytic(baud_rate, delta_frequency, alpha, beta2, gamma, length, spm_weight, xpm_weight):
    ...
    @staticmethod
    def _ggn_spectrally_separated(cut_indices, frequency, signal, baud_rate, roll_off, slot_width,
                                  srs: StimulatedRamanScattering, alpha, beta2, beta3, f_ref_beta, gamma,
                                  spm_weight, xpm_weight):
        """Computes the nonlinear interference power evaluated at the fiber input.
        ...
        eta_cut_central_frequency = gamma ** 2 * weight * psi_cut_central_frequency / (
                    cut_baud_rate * pump_baud_rate ** 2)
        eta = cut_baud_rate * eta_cut_central_frequency  # Local white noise
        return eta
    ...
    @staticmethod
    def _generalized_psi(f_eval, cut_frequency, cut_baud_rate, cut_roll_off, pump_frequency, pump_baud_rate, pump_roll_off,
                         f_cut_resolution, f_pump_resolution, srs, alpha, beta2, beta3, f_ref_beta):
        """Computes the generalized psi function similarly to the one used in the GN model."""
        z = srs.z
        rho_norm = srs.rho * exp(outer(alpha / 2, z))
        rho_pump = interp1d(srs.frequency, rho_norm, axis=0)(pump_frequency)
