    ...
    def calculate_unidirectional_stimulated_raman_scattering(power_in, alpha, cr, z, lumped_losses):
        ...
        if sim_params.raman_params.method == 'perturbative':
            ...
            for i in range(1, z.size):
                dz = z[i] - z[i-1]
                p0 = power[:, i - 1] * lumped_losses[i - 1]
                alphaz = alpha * dz
                expz = exp(- alphaz)
                eff_length = 1 / alpha * (1 - expz)
                exponent = - alphaz
                for order in range(1, sim_params.raman_params.order + 1):
                    if order == 1:
                        gamma1 = sum(cr * p0 * eff_length, 1)
                        exponent += gamma1
                    elif order == 2:
                        gamma2 = sum(cr * p0 * expz * gamma1 * dz, 1)
                        exponent += gamma2
                    elif order == 3:
                        gamma3 = sum(cr * p0 * expz * (gamma2 + 1 / 2 * gamma1 ** 2) * dz, 1)
                        exponent += gamma3
                    elif order == 4:
                        gamma4 = sum(
                            cr * p0 * expz * (gamma3 + gamma1 * gamma2 + 1 / factorial(3) * gamma1 ** 3) * dz, 1)
                        exponent += gamma4
                    else:
                        raise ValueError(f'Order {order} not implemented in Raman Solver.')
                    power[:, i] = p0 * exp(exponent)
...
class NliSolver:
    ...
    'ggn_approx': eq. 24-25 https://ieeexplore.ieee.org/document/9741324
    """
                ...
                if cut_index == pump_index:  # SPM
                    psi_cut_central_frequency[i, pump_index] = \
                        NliSolver._generalized_psi(cut_frequency, cut_frequency, cut_baud_rate, cut_roll_off,
                                                   ...
                                                   f_pump_resolution, srs, pump_alpha, cut_beta2, 0,
                                                   cut_frequency)
                else:  # XPM
                    ...
                    if abs(delta_f) <= frequency_offset_threshold:
                        ...
                        psi_cut_central_frequency[i, pump_index] = \
                            NliSolver._fast_generalized_psi(cut_frequency, cut_frequency, cut_baud_rate, cut_roll_off,
                                                            ...
                                                            f_cut_resolution, srs, pump_alpha, cut_beta2, 0,
                                                            cut_frequency)
