...
|                      |           | gain are set in amplifier, auto_design    |
...
|                      |           | auto_design power optimisation range      |
...
keywords are:
    ...
    - (user defined power offset depends on -spectrum option inputs)

Each ROADM instance may have its own equalization choice.
...
means that target power is -20 dBm for all degrees except "edfa in
...
means that roadm A uses power equalization on all its degrees except
"edfa in roadm A to toto" where it is power_sectral density.
