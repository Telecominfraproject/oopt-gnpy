...
the ``roadm-path-impairments`` list allows the definition of impairments list per internal path category
(add, drop or express). several add-path drop-path or expresss-path can be defined. They are indexed and the impairments defined per band.
...
+----------------------------+-----------+-----------------------------------------------------------+-------------+-------------+---------------------+
| ``roadm-pmin``             |           | Min per carrier power levels expected at the output of    | Not yet     | N.A.        | N.A.                |
|                            |           | the drop block.                                           | implemented |             |                     |
+----------------------------+-----------+-----------------------------------------------------------+-------------+-------------+---------------------+
...
+----------------------------+-----------+-----------------------------------------------------------+-------------+-------------+---------------------+
| ``roadm-pmd``              | (number)  | PMD contribution of this roadm path.                      | Implemented | Implemented | Implemented         |
+----------------------------+-----------+-----------------------------------------------------------+-------------+-------------+---------------------+
| ``roadm-cd``               |           |                                                           | Not yet     | Not yet     | Not yet             |
|                            |           |                                                           | Implemented | Implemented | Implemented         |
+----------------------------+-----------+-----------------------------------------------------------+-------------+-------------+---------------------+
| ``roadm-pdl``              | (number)  | PMD contribution of this roadm path.                      | Implemented | Implemented | Implemented         |
+----------------------------+-----------+-----------------------------------------------------------+-------------+-------------+---------------------+
...
On this example, this means that express channel will have at least 16.5dB loss when crossing this ROADM express path with this impairment id.

roadm-path-impairments is optional. If present, its values are considered instead of the ROADM general parameters.
For example if add-path specifies 0.5 dB PDL and general PDL parameter states 1dB, then 0.5dB is applied for
this roadm-path only.
...
.. math::
  ...
  add_{osnr} = drop_{osnr} = add_{osnr} + 10log10(2)

User can specify the roadm type_variety in json topology ROADM instance. If no variety is defined ``default`` ID is used.
User can defined the impairment type for each raodm-path using the degrees ingress/egress immediate neighbour elements and
the roadm-path-impairment-id defined in the library for this type-variety.
Here is an example:
...
If nothing is stated for impairments on roadm-paths, the program identifies the paths implicitely and assign the first
impairment_id that matches the type: if a transceiver is present on one degree then it is an add/drop degree.
...
| ``tx_osnr``          | (number)  | In dB. OSNR out from transponder.         |
+----------------------+-----------+-------------------------------------------+
| ``tx_power_dbm``     | (number)  | In dBm. Power out from transceiver.       |
|                      |           | Optional. Default = power_dbm             |
+----------------------+-----------+-------------------------------------------+
...
User can specify configuration of amplifiers. Configurations are applied depending on general simulation setup:
- if user has specified ``power_mode`` True in Span section, delta_p are applied and gain_target are ignored and recomputed.
- if user has specified ``power_mode`` False in Span section, gain_target are applied and delta_p are ignored.
If user has specified unfeasible targets with respect to the type_variety, the targets might be changed accordingly,
for example if the gain_target leads to power value above max output power of the amplifier, gain is saturated to
...
| or                                     |           | for this ROADM. Optional: if not defined, use the  |
| ``target_psd_out_mWperGHz``            |           | one defined in library for this type_variety.      |
| or                                     |           |                                                    |
| ``target_out_mWperSlotWidth``          |           |                                                    |
| (mutually exclusive)                   |           |                                                    |
+----------------------------------------+-----------+----------------------------------------------------+
| ``restrictions``                       | (dict of  | Optional. If defined, overides restriction         |
|                                        |  strings) | defined in library for this roadm type_variety.    |
+----------------------------------------+-----------+----------------------------------------------------+
| ``per_degree_pch_out_db``              | (dict of) | Optional. If defined, overides ROADM's general     |
| or                                     |  string,  | target power/psd for this degree. Dictionnary with |
...
|                                        | dict)     | defines it overrides the general values defined    |
|                                        |           | by type_variety.                                   |
+----------------------------------------+-----------+----------------------------------------------------+
...
In this example, all «implicit» express roadm-path are assigned roadm-path-impairment-id = 0,
and target power is set to the library defined value except for the directionn heading to "east edfa in SITE1 to ILA1",
where constant power equalization is used with target power set to -13.5dBm.

Fused
~~~~~

User can define concentrated losses thanks to Fused element. This can be useful for example to materialize connector with its loss between two fiber spans.
...
Transceiver elements represent the logical function that generates a spectrum. This must be specified to start
and stop propagation. However the characteristics of the spectrum is defined elsewhere, so Transceivers do not contain any attribute.
Information on used transceivers' type, modes and frequency must be listed in :ref:`service file<service>` or :ref:`spectrum file<mixed-rate>`. Without any definition,
default :ref:`SI<spectral_info>` values of the library are propagated.
...
+-----------------------+------------+----------------------------------------------------------------+
| ``source``            | (string)   | Mandatory. Source of traffic. Must be one of the UID of        |
|                       |            | transceivers listed in the topology.                           |
+-----------------------+------------+----------------------------------------------------------------+
| ``src-tp-id``         | (string)   | Mandatory. Must be equal to source.                            |
+-----------------------+------------+----------------------------------------------------------------+
| ``destination``       | (string)   | Mandatory. Destination of traffic. Must be one of the UID of   |
|                       |            | transceivers listed in the topology.                           |
+-----------------------+------------+----------------------------------------------------------------+
| ``dst-tp-id``         | (string)   | Mandatory. Must be equal to destination.                       |
+-----------------------+------------+----------------------------------------------------------------+
| ``bidirectional``     | (boolean)  | Mandatory. boolean inidicating if the propagation should be    |
|                       |            | checked on source-destination only (false) or also on          |
|                       |            | destination-source (true).                                     |
+-----------------------+------------+----------------------------------------------------------------+
| ``path-constraints``  | (dict)     | Mandatory. Contains the list of constraints including type of  |
|                       |            | transceiver, mode and nodes to be included in the path.        |
+-----------------------+------------+----------------------------------------------------------------+
...
+-----------------------------------+------------+----------------------------------------------------------------+
| ``trx_mode``                      | (string)   | Optional. mode selected for this path. It MUST be listed in    |
|                                   |            | the transceiver's mode in the library. If not defined, the     |
|                                   |            | gnpy-path-request script automatically selects the mode that   |
|                                   |            | has performance above minimum required threshold including     |
|                                   |            | margins and penalties for all channels (full load) and         |
|                                   |            | 1) fit in the spacing, 2) has the largest baudrate,            |
|                                   |            | 3) has the largest bitrate.                                    |
+-----------------------------------+------------+----------------------------------------------------------------+
| ``spacing``                       | (number)   | Mandatory. in :math:`Hz`. Spacing in used for full load        |
|                                   |            | evaluation of the feasibility.                                 |
+-----------------------------------+------------+----------------------------------------------------------------+
| ``path_bandwidth``                | (number)   | Mandatory. In :math:`bit/s`. Required capacity on this         |
|                                   |            | service. It is used to determine the nb of channels needed     |
|                                   |            | and the spectrum occupation.                                   |
+-----------------------------------+------------+----------------------------------------------------------------+
| ``max-nb-of-channel``             | (number)   | Optional. Nb of channel to take into account for the full      |
...
|                                   |            | amplifiers. This parameter can thus be usefull to test         |
|                                   |            | different design with the same script.                         |
|                                   |            |                                                                |
|                                   |            | In order to keep the same design for different requests,       |
|                                   |            | ``max-nb-of-channel` and ``output-power`` should be identical  |
+-----------------------------------+------------+----------------------------------------------------------------+
| ``tx_power``                      | (number)   | Optional. In :math:`W`. Power out from transceiver.            |
|                                   |            | Default value is output-power.                                 |
+-----------------------------------+------------+----------------------------------------------------------------+
...
|                                   |            | Any combination of frequency slots is allowed as long as no    |
|                                   |            | two frequency slots overlap.                                   |
...
|                                   |            | Note that gnpy-path-request script uses full load and not this |
|                                   |            | spectrum constraint to compute performance. Thus, The specific |
|                                   |            | mix of channels resulting from the list of requests is not     |
|                                   |            | considered to compute performances.                            |
+-----------------------------------+------------+----------------------------------------------------------------+
| ``route-object-include-exclude``  | (list)     | Optional. Indexed List of routing include constraints to       |
...
| field                             |   type     | description                                                    |
+===================================+============+================================================================+
| ``explicit-route-usage``          | (string)   | Mandatory. Only one supported value : ``route-include-ero``    |
+-----------------------------------+------------+----------------------------------------------------------------+
